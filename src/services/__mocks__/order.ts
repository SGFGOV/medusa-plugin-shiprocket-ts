...
const fulfillmentTypesDomestic = [
...
export const orders = {
    testOrderIndiaDomestic: {
        ...
        region_id: IdMap.getId("region-india"),
        customer_id: IdMap.getId("test-customer"),
        payment_method: {
            provider_id: "default_provider"
        },
        shipping_methods: fulfillmentTypesDomestic.map((f, index) => {
            return {
                _id: IdMap.getId(f),
                name: f,
                price: 100 * (index + 1),
                provider_id: "shiprocket",
                profile_id: IdMap.getId("shiprocket"),
                data: {
                    extra: "hi"
                }
            };
...
export const OrderModelMock = {
    ...
    createShipment: jest.fn().mockReturnValue(Promise.resolve())
};
